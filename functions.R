...
runModelAdapt <- function(deltaID,sampleID=1, climScen=0, outType="dTabs",rcps = "CurrClim",
  ...
  if(rcpfile=="CurrClim"){
    ...
    rm(list = "xday")
      ...
      datname <- load(paste0(workdir, "fmi_vars_PREBAS.rdata"))
      assign("dat",get(datname))
      lookupname <- load(paste0(workdir, "climID_lookup.rdata"))
  ...
  initPrebas = create_prebas_input_tmp.f(r_no, clim, data.sample, 
                                         nYears = nYears,harv=harvScen,
                                         startingYear = startingYear,
                                         domSPrun=domSPrun,
                                         HcFactorX=HcFactor, 
                                         climScen=climScen, 
                                         sampleX=sampleX, #clcut = clcut, 
    ...
    if(harvScen=="baseTapio"){
      ...
      if (is.na(minDharvX)) {
        ...
        save(initPrebas, HarvLimX, minDharvX,cutArX,compHarvX,
             file=paste0("/scratch/project_2000994/PREBASruns/PREBAStesting/testRun",rcps,".rdata"))
